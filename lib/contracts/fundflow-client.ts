...
const contract = STELLAR_CONFIG.contractId
  ? new Contract(STELLAR_CONFIG.contractId)
  : null;
...
function hasContract() {
  return Boolean(
    contract &&
      ...
      STELLAR_CONFIG.contractId !== 'CONTRACT_ADDRESS_HERE'
  );
}
...
function requireContract() {
  if (!hasContract() || !contract) {
    throw new Error(
      'NEXT_PUBLIC_CONTRACT_ID is not configured. Deploy the Soroban contract and update the environment first.'
    );
  }
...
export async function fetchCampaign(
  ...
  const value = await readContract('get_campaign', [
  ...
  return response.events
    .map((event, index) => {
      const topic = event.topic
        .map((item) => String(scValToNative(item)))
        .join(':');
      ...
      const type = topic.includes('created')
        ? 'CampaignCreated'
        : topic.includes('donated')
        ? 'DonationMade'
        : 'FundsWithdrawn';
...
export async function createCampaign(input: {
  ...
  return invokeContract(input.publicKey, 'create_campaign', [
    ...
    nativeToScVal(
      BigInt(
        Math.round(
          Number(input.fundingGoal) * 10_000_000
        )
      ),
      { type: 'i128' }
    ),
    nativeToScVal(input.deadline, {
      type: 'u64'
    })
  ]);
}

export async function donate(input: {
  publicKey: string;
  campaignId: number;
  amount: string;
}) {
  return invokeContract(input.publicKey, 'donate', [
    new Address(input.publicKey).toScVal(),
    nativeToScVal(input.campaignId, {
      type: 'u32'
    }),
    nativeToScVal(
      BigInt(
        Math.round(Number(input.amount) * 10_000_000)
      ),
      { type: 'i128' }
    )
  ]);
...
export async function withdraw(input: {
  ...
  return invokeContract(input.publicKey, 'withdraw', [
    new Address(input.publicKey).toScVal(),
    nativeToScVal(input.campaignId, {
      type: 'u32'
    })
